...
// ***************************************************************************************************
// CMSMS Specific : include the libs, and then see if the current users profile can use the file manager
$DONT_LOAD_SMARTY = true;
$CMS_ADMIN_PAGE=1;

// Dirs are different in CMSMS < 2.3 and 2.3 so we try to load both
// and we must take care of java upload dialog which is in a subdir too. So there are 3 paths options to go to the CMSMS include:
if (file_exists('../../../../lib/include.php')) {
    @require_once('../../../../lib/include.php');
} elseif (file_exists('../../../../../lib/include.php')) {
    @require_once('../../../../../lib/include.php');
} else {
    @require_once('../../../../../../lib/include.php');
}
// if (basename($_SERVER["SCRIPT_FILENAME"]) != 'index.php') { // index.php = uploader/index.php
//     require_once('../../../../lib/include.php');
// } else {
//     require_once('../../../../../lib/include.php');
// }
...
$can_use_filemanager = tinymce_utils::can_user_use_filemanager($profile);


if (!$can_use_filemanager) {
    die('Access Denied!');
} else {
    // Load the config
    $filemanager_config = $profile->get_filemanager_config($filemanager_config);
}
// END CMSMS SPECIFIC CODE
// ***************************************************************************************************
...
return array_merge(
    ...
    array(
        'ext' => array_merge(
            $filemanager_config['ext_img'],
            $filemanager_config['ext_file'],
            $filemanager_config['ext_misc'],
            $filemanager_config['ext_video'],
            $filemanager_config['ext_music']
        ),
        'tui_defaults_config' => array(
            //'common.bi.image'                   => $filemanager_config['common.bi.image'],
            //'common.bisize.width'               => $filemanager_config['common.bisize.width'],
            //'common.bisize.height'              => $filemanager_config['common.bisize.height'], 
            'common.backgroundImage'            => $filemanager_config['common.backgroundImage'],
            'common.backgroundColor'            => $filemanager_config['common.backgroundColor'], 
            'common.border'                     => $filemanager_config['common.border'],
            'header.backgroundImage'            => $filemanager_config['header.backgroundImage'],
            'header.backgroundColor'            => $filemanager_config['header.backgroundColor'],
            'header.border'                     => $filemanager_config['header.border'],
            'menu.normalIcon.path'              => $filemanager_config['menu.normalIcon.path'],
            'menu.normalIcon.name'              => $filemanager_config['menu.normalIcon.name'],
            'menu.activeIcon.path'              => $filemanager_config['menu.activeIcon.path'],
            'menu.activeIcon.name'              => $filemanager_config['menu.activeIcon.name'],
            'menu.disabledIcon.path'            => $filemanager_config['menu.disabledIcon.path'],
            'menu.disabledIcon.name'            => $filemanager_config['menu.disabledIcon.name'],
            'menu.hoverIcon.path'               => $filemanager_config['menu.hoverIcon.path'],
            'menu.hoverIcon.name'               => $filemanager_config['menu.hoverIcon.name'],
            'menu.iconSize.width'               => $filemanager_config['menu.iconSize.width'],
            'menu.iconSize.height'              => $filemanager_config['menu.iconSize.height'],
            'submenu.backgroundColor'           => $filemanager_config['submenu.backgroundColor'],
            'submenu.partition.color'           => $filemanager_config['submenu.partition.color'],
            'submenu.normalIcon.path'           => $filemanager_config['submenu.normalIcon.path'],
            'submenu.normalIcon.name'           => $filemanager_config['submenu.normalIcon.name'],
            'submenu.activeIcon.path'           => $filemanager_config['submenu.activeIcon.path'],
            'submenu.activeIcon.name'           => $filemanager_config['submenu.activeIcon.name'],
            'submenu.iconSize.width'            => $filemanager_config['submenu.iconSize.width'],
            'submenu.iconSize.height'           => $filemanager_config['submenu.iconSize.height'],
            'submenu.normalLabel.color'         => $filemanager_config['submenu.normalLabel.color'],
            'submenu.normalLabel.fontWeight'    => $filemanager_config['submenu.normalLabel.fontWeight'],
            'submenu.activeLabel.color'         => $filemanager_config['submenu.activeLabel.color'],
            //'submenu.activeLabel.fontWeight'    => $filemanager_config['submenu.activeLabel.fontWeightcommon.bi.image'],
            'checkbox.border'                   => $filemanager_config['checkbox.border'],
            'checkbox.backgroundColor'          => $filemanager_config['checkbox.backgroundColor'],
            'range.pointer.color'               => $filemanager_config['range.pointer.color'],
            'range.bar.color'                   => $filemanager_config['range.bar.color'],
            'range.subbar.color'                => $filemanager_config['range.subbar.color'],
            'range.disabledPointer.color'       => $filemanager_config['range.disabledPointer.color'],
            'range.disabledBar.color'           => $filemanager_config['range.disabledBar.color'],
            'range.disabledSubbar.color'        => $filemanager_config['range.disabledSubbar.color'],
            'range.value.color'                 => $filemanager_config['range.value.color'],
            'range.value.fontWeight'            => $filemanager_config['range.value.fontWeight'],
            'range.value.fontSize'              => $filemanager_config['range.value.fontSize'],
            'range.value.border'                => $filemanager_config['range.value.border'],
            'range.value.backgroundColor'       => $filemanager_config['range.value.backgroundColor'],
            'range.title.color'                 => $filemanager_config['range.title.color'],
            'range.title.fontWeight'            => $filemanager_config['range.title.fontWeight'],
            'colorpicker.button.border'         => $filemanager_config['colorpicker.button.border'],
            'colorpicker.title.color'           => $filemanager_config['colorpicker.title.color']
        ),
    )
);